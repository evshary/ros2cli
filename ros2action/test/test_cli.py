...
class TestROS2ActionCLI(unittest.TestCase):
    ...
    def setUpClass(
        ...
        def launch_action_command(self, arguments):
            action_command_action = ExecuteProcess(
                cmd=['ros2', 'action', *arguments], name='ros2action-cli', output='screen',
                additional_env={'RMW_IMPLEMENTATION': rmw_implementation, 'PYTHONUNBUFFERED': '1'}
            )
            with launch_testing.tools.launch_process(
                launch_service, proc_info, proc_output, action_command_action,
    ...
    def test_list(self):
        ...
        assert action_command.exit_code == launch_testing.asserts.EXIT_OK
        assert launch_testing.tools.expect_output(
            expected_output=['/fibonacci'], output=action_command.output
        )
    ...
    def test_list_with_types(self):
        ...
        assert action_command.exit_code == launch_testing.asserts.EXIT_OK
        assert launch_testing.tools.expect_output(
            expected_output=['/fibonacci [test_msgs/action/Fibonacci]'],
            output=action_command.output
        )
    ...
    def test_send_fibonacci_goal_with_feedback(self):
        ...
        assert action_command.exit_code == launch_testing.asserts.EXIT_OK
        assert launch_testing.tools.expect_output(
            expected_output=get_fibonacci_send_goal_output(
