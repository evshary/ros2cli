...
class TestROS2ServiceCLI(unittest.TestCase):
    ...
    def setUpClass(
        ...
        def launch_service_command(self, arguments):
            service_command_action = ExecuteProcess(
                cmd=['ros2', 'service', *arguments], name='ros2service-cli', output='screen',
                additional_env={'RMW_IMPLEMENTATION': rmw_implementation, 'PYTHONUNBUFFERED': '1'}
            )
            with launch_testing.tools.launch_process(
                launch_service, proc_info, proc_output, service_command_action,
    ...
    def test_list_hidden(self):
        with self.launch_service_command(
            ...
            assert service_command.wait_for_shutdown(timeout=10)
        assert services_command.exit_code == launch_testing.asserts.EXIT_OK
        assert launch_testing.tools.expect_output(
            expected_output=itertools.chain(
                ['/my_ns/_echo', '/my_ns/echo']
                itertools.repeat(re.compile(
    ...
    def test_find(self):
        ...
        assert service_command.exit_code == launch_testing.asserts.EXIT_OK
        assert launch_testing.tools.expect_output(
            expected_output=['/my_ns/echo'], output=service_command.output
        )
    ...
    def test_find_hidden(self):
        ...
        assert service_command.exit_code == launch_testing.asserts.EXIT_OK
        assert launch_testing.tools.expect_output(
            expected_output=['/my_ns/echo', '/my_ns/echo'],
            output=service_command.output
        )

    @launch_testing.markers.retry_on_failure(times=5)
    def test_find_count(self, rmw_output_filter):
        with self.launch_service_command(
    ...
    def test_find_not_a_service_type(self, rmw_output_filter):
        with self.launch_service_command(
    ...
    def test_type(self):
        ...
        assert service_command.exit_code == launch_testing.asserts.EXIT_OK
        assert launch_testing.tools.expect_output(
            expected_output=['test_msgs/srv/BasicTypes'],
            output=service_command.output
        )
    ...
    def test_call_no_args(self):
        ...
        assert service_command.exit_code == launch_testing.asserts.EXIT_OK
        assert launch_testing.tools.expect_output(
            expected_output=get_echo_call_output(),
            output=service_command.output
        )
