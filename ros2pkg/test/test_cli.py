...
class TestROS2PkgCLI(unittest.TestCase):
    ...
    def setUpClass(
        ...
        def launch_pkg_command(self, arguments, **kwargs):
            pkg_command_action = ExecuteProcess(
                ...
                name='ros2pkg-cli', output='screen',
                additional_env={'PYTHONUNBUFFERED': '1'},
                **kwargs
            )
            with launch_testing.tools.launch_process(
                launch_service, proc_info, proc_output, pkg_command_action
            ) as pkg_command:
                yield pkg_command
    ...
    def test_xml(self):
        with self.launch_pkg_command(arguments=['xml', 'ros2cli']) as pkg_command:
            assert pkg_command.wait_for_shutdown(timeout=2)
        assert pkg_command.exit_code == launch_testing.asserts.EXIT_OK
        root = ET.XML(msg_command.output)
        assert root.tag == 'package'
        assert root.find('name').text == 'ros2cli'

    def test_nonexistent_package_xml(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert launch_testing.tools.expect_output(
                expected_output=[
                    ...
                    'package format: 3',
                    'version: 0.0.0',
                    'description: A test dummy description',
            ...
            assert os.path.isfile(os.path.join(tmpdir, 'a_test_package', 'src', 'test_node.cpp'))
            assert os.path.isfile(os.path.join(tmpdir, 'a_test_package', 'src', 'test_library.cpp'))
            ...
            # Check package.xml
            root = ET.parse(os.path.join(tmpdir, 'a_test_package', 'package.xml'))
            assert root.tag == 'package'
            assert root.attrib['format'] == '3'
            assert root.find('name').text == 'a_test_package'
            assert root.find('description').text == 'A test dummy description'
